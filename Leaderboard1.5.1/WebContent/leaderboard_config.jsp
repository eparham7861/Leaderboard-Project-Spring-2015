...
<%
	...
	application.setAttribute("theCourseID", courseID);
	try {
	...
	String color_value = currentXML.getContent(SavedContent.Content.OTHERCOLOR);
	String user_color_value = currentXML.getContent(SavedContent.Content.USERCOLOR);
	...
	for(int i = 0; i < 10; i ++){
		level_values[i] = currentXML.getLevelValue(i);
		level_labels[i] = currentXML.getLevelName(i);
	...
	<bbNG:dataCollection>
			...
			<%	
				...
				if (sessionUserRole.trim().toLowerCase().equals("instructor")) {
					...
					color_value = currentXML.getContent(SavedContent.Content.COURSECOLOR);
				}	
			%>
			...
			<% if (isUserAnInstructor) { %>
				...
				*/
				...
				if(modified.equals("true")){//A save file already exists.
					...
					String visibleList = currentXML.getContent(SavedContent.Content.VISIBLE);
					...
					String hiddenList = currentXML.getContent(SavedContent.Content.HIDDEN);
				...
				prev_grade_choice = currentXML.getContent(SavedContent.Content.GRADECHOICE);
